...
| "no_canonical_match" | "Web URI where security.txt is located must match with a 'Canonical' field. In case of redirecting either the first or last web URI of the redirect chain must match. (Note that the security.txt specification is not clear regarding the latter.)
